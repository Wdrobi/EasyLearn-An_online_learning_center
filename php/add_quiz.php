...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '') {
        ...
        if ($stmt->execute()) {
            ...
            $stmt->close();
            $conn->close();
            header('Location: manage_quiz_questions.php?quiz_id=' . $quiz_id . '&course_id=' . $course_id);
            exit();
        } else {
...
<div class="container" style="margin-top: 90px;">
    <div class="dashboard card" style="padding: 2.5rem 2rem 2rem 2rem; margin-top: 2rem; max-width: 600px; margin-left:auto; margin-right:auto;">
        <h1 style="margin-bottom: 1.5rem;">Add Quiz to <?php echo htmlspecialchars($course['title']); ?></h1>
