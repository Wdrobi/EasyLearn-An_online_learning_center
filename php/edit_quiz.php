...
if (!isset($_GET['id']) || !isset($_GET['course_id'])) { header('Location: instructor_dashboard.php'); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '') {
        ...
        if ($stmt->execute()) {
            $stmt->close();
            $conn->close();
            header('Location: manage_quizzes.php?course_id=' . $course_id);
            exit();
        } else {
...
<div class="container" style="margin-top: 90px;">
    <div class="dashboard card" style="padding: 2.5rem 2rem 2rem 2rem; margin-top: 2rem; max-width: 600px; margin-left:auto; margin-right:auto;">
        <h1 style="margin-bottom: 1.5rem;">Edit Quiz for <?php echo htmlspecialchars($quiz['course_title']); ?></h1>
        ...
        <form method="post" class="auth-form">
            ...
            <div class="form-actions">
                ...
                <a href="manage_quizzes.php?course_id=<?php echo $course_id; ?>" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
